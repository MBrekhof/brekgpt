...
namespace brekGPT.Module.E2E.Tests;

public class brekGPTTests : IDisposable {
    const string BlazorAppName = "brekGPTBlazor";
    const string WinAppName = "brekGPTWin";
    const string AppDBName = "brekGPT";
    EasyTestFixtureContext FixtureContext { get; } = new EasyTestFixtureContext();

	public brekGPTTests() {
        FixtureContext.RegisterApplications(
            new BlazorApplicationOptions(BlazorAppName, string.Format(@"{0}\..\..\..\..\brekGPT.Blazor.Server", Environment.CurrentDirectory)),
            new WinApplicationOptions(WinAppName, string.Format(@"{0}\..\..\..\..\brekGPT.Win\bin\EasyTest\net6.0-windows\brekGPT.Win.exe", Environment.CurrentDirectory))
        );
        FixtureContext.RegisterDatabases(new DatabaseOptions(AppDBName, "brekGPTEasyTest", server: @"(localdb)\mssqllocaldb"));	           
	}
    public void Dispose() {
